...
class SupplierCatalogueAdapter(
    private var catalogues: List<Catalogue>,
    private val onItemClick: (Catalogue) -> Unit
) : RecyclerView.Adapter<SupplierCatalogueAdapter.CatalogueViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CatalogueViewHolder {
        val binding =
            BuyerCatalogueCardBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return CatalogueViewHolder(binding)
    }

    override fun onBindViewHolder(
        holder: SupplierCatalogueAdapter.CatalogueViewHolder,
        position: Int
    ) {
